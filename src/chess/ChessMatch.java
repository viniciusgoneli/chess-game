...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String piece) {
		...
		if(!piece.equals("Q") && !piece.equals("N") && !piece.equals("B") && !piece.equals("R")) {
			return promoted;
		}
		Position pos = promoted.getChessPosition().toPosition();
		ChessPiece p = (ChessPiece)board.removePiece(pos);
		_piecesOnTheBoard.remove(p);
		ChessPiece newPiece = newPiece(piece, p.getColor());
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {																// exista.
		...
		ChessPiece movedPiece = (ChessPiece)board.piece(tarjet);
		
		//specialmove promotion
		if(movedPiece instanceof Pawn) {
			Position promotionPos;
			if(movedPiece.getColor() == Color.WHITE && tarjet.getRow() == 0 || movedPiece.getColor() == Color.BLACK && tarjet.getRow() == 7) {
				promoted = (ChessPiece)board.piece(tarjet);
		...
		if(movedPiece instanceof Pawn && (tarjet.getRow() == source.getRow() - 2 || tarjet.getRow() == source.getRow()+2)) {
	...
	private Piece makeMove(Position source, Position tarjet) {
		...
		//specialmove castiling kingside move
		if(tarjet.getColumn() == source.getColumn() + 2 && p instanceof King) {
		...
		if(p instanceof Pawn) {
			if(source.getColumn() != tarjet.getColumn() && capturedPiece == null) {
				Position pos;
				if(p.getColor() == Color.WHITE) {
					pos = new Position(tarjet.getRow() + 1, tarjet.getColumn());
	...
	private void undoMove(Position source, Position tarjet, Piece capturedPiece) {
		ChessPiece p = (ChessPiece)board.removePiece(tarjet);
		p.decreaseMoveCount();
		...
		//specialmove castiling queenside move
		if(tarjet.getColumn() == source.getColumn() + 2 && p instanceof King) {
			Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
			Position targetT = new Position(source.getRow(), source.getColumn() + 1);
		...
		if(p instanceof Pawn) {
			if(source.getColumn() != tarjet.getColumn() && capturedPiece == enPassantVunerable) {
				ChessPiece pawn = (ChessPiece)board.removePiece(tarjet);
				Position pawnPosition;
				if(p.getColor() == Color.WHITE) {
					pawnPosition = new Position(3, tarjet.getColumn());
					
				}else {
					pawnPosition = new Position(4, tarjet.getColumn());
				}
